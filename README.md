...
| `GET` | `/api/boardgames/{id}` | Get board game by ID | 200 OK | 404 Not Found |
| `POST` | `/api/boardgames` | Create a new board game | 201 Created | — |
| `PUT` | `/api/boardgames/{id}` | Update a board game | 204 No Content | 404 Not Found |
| `DELETE` | `/api/boardgames/{id}` | Delete a board game | 204 No Content | 404 Not Found |
...
4. Run the requests in order (1 → 6 → 9 → 2 → 3 → 4 → 5 → 7 → 8)
